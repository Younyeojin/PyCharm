...
class Plot(object):
    ...
    def show_plot_survived_dead(self):
        this = self.df
        f, ax = plt.subplots(1, 2, figsize= (18,8))   # subplots:1행 2열
        series = this['Survived'].value_counts()
        print(type(series))
        print(series)
        series.plot.pie(explode=[0, 0.1], autopct='%1.1f%%', ax=ax[0], shadow=True) # ax:축
    ...
    def show_plot_embarked(self):
        ...
        sns.countplot('Embarked', data=this, x='승선항구', hue='생존결과')
        plt.show()